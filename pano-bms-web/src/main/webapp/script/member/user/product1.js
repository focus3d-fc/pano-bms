$(function() {
	...
	$('#fileupload0').fileupload({
			...
			$("#files00").children().remove();
			$('#progress0 .progress-bar').css('width', 0 + '%');
			data.context = $('<div/>').appendTo('#files00');
		...
		$.each(data.result, function(index, file){
			var txt = $("#resultInfo00").text();
			// alert(txt);
			$("#fullimgsn01").val(txt.substr(1));
			$("#resultInfo00").text(txt + "," + file);
	...
	// 第二张上传
	var uploadButton1 = $('<button/>').addClass('btn btn-primary').prop(
			'disabled', true).text('Processing...').on('click', function() {
		var $this = $(this);
		var data = $this.data();
		$this.off('click').text('Abort').on('click', function() {
	...
	$('#fileupload01').fileupload(
		...
		$('#progress01 .progress-bar').css('width', 0 + '%');
		...
		var progress = parseInt(data.loaded / data.total * 100, 10);
		$('#progress01 .progress-bar').css('width', progress + '%');
	}).on('fileuploaddone', function(e, data) {
		$("#resultInfo01").text("");
		var returnJsonAry = data.result;
		$.each(data.result, function(index, file) {
	...
	// 第3张上传
	var uploadButton2 = $('<button/>').addClass('btn btn-primary').prop(
			'disabled', true).text('Processing...').on('click', function() {
		var $this = $(this);
		var data = $this.data();
		$this.off('click').text('Abort').on('click', function() {
	...
	$('#fileupload02').fileupload(
		...
		$('#progress02 .progress-bar').css('width', 0 + '%');
		...
		var progress = parseInt(data.loaded / data.total * 100, 10);
		$('#progress02 .progress-bar').css('width', progress + '%');
	}).on('fileuploaddone', function(e, data) {
		$("#resultInfo02").text("");
		var returnJsonAry = data.result;
		$.each(data.result, function(index, file) {
	...
	// 第4张上传
	var uploadButton3 = $('<button/>').addClass('btn btn-primary').prop(
			'disabled', true).text('Processing...').on('click', function() {
		var $this = $(this);
		var data = $this.data();
		$this.off('click').text('Abort').on('click', function() {
	...
	$('#fileupload03').fileupload(
		...
		$('#progress03 .progress-bar').css('width', 0 + '%');
		...
		var progress = parseInt(data.loaded / data.total * 100, 10);
		$('#progress03 .progress-bar').css('width', progress + '%');
	}).on('fileuploaddone', function(e, data) {
		$("#resultInfo03").text("");
		var returnJsonAry = data.result;
		$.each(data.result, function(index, file) {
	...
	// 第5张上传
	var uploadButton4 = $('<button/>').addClass('btn btn-primary').prop(
			'disabled', true).text('Processing...').on('click', function() {
		var $this = $(this);
		var data = $this.data();
		$this.off('click').text('Abort').on('click', function() {
	...
	$('#fileupload04').fileupload(
		...
		$('#progress04 .progress-bar').css('width', 0 + '%');
		...
		var progress = parseInt(data.loaded / data.total * 100, 10);
		$('#progress04 .progress-bar').css('width', progress + '%');
	}).on('fileuploaddone', function(e, data) {
		$("#resultInfo04").text("");
		var returnJsonAry = data.result;
		$.each(data.result, function(index, file) {
	...
	// 第六张图片
	var uploadButton5 = $('<button/>').addClass('btn btn-primary').prop(
			'disabled', true).text('Processing...').on('click', function() {
		var $this = $(this);
		var data = $this.data();
		$this.off('click').text('Abort').on('click', function() {
	...
	$('#fileupload05').fileupload(
		...
		$("#files05").children().remove();
		$('#progress05 .progress-bar').css('width', 0 + '%');

		data.context = $('<div/>').appendTo('#files05');
		$.each(data.files, function(index, file) {
			var node = $('<p/>').append($('<span/>').text(file.name));
			if (!index) {
				/*
		...
		var progress = parseInt(data.loaded / data.total * 100, 10);
		$('#progress05 .progress-bar').css('width', progress + '%');
	}).on('fileuploaddone', function(e, data) {
		$("#resultInfo05").text("");
		var returnJsonAry = data.result;
		$.each(data.result, function(index, file) {
			// alert(index + "," + file);
			var txt = $("#resultInfo05").text();
			$("#longimgsn01").val(txt.substr(1));
			$("#resultInfo05").text(txt + "," + file);
		});
		$("#resultInfo05").text("");
	}).on(
			'fileuploadfail',
