...
public class PerspectiveQuery extends BaseController {
	...
	public void QueryViewInfo(HttpServletResponse response,PanoPerspectiveViewModel model) {
		try {
	...
	public void QueryViewProducts(HttpServletResponse response,
		...
		try {
			...
			/*
			for (LinkedHashMap<String, Object> map : list) {
				LinkedHashMap<String, Object> view;
				LinkedHashMap<String, Object> view_child;

				String viewId = map.get("viewId").toString();

				if (space.containsKey(viewId)) {
					view = (LinkedHashMap<String, Object>)space.get(viewId);
					view_child = (LinkedHashMap<String, Object>)view.get("layer");
				} else {
					view = new LinkedHashMap<String, Object>();
					space.put(viewId, view);

					view.put("id", viewId);
					view.put("name", map.get("viewName").toString());
					...
					view.put("height",
							client.getFile(mapKey, FileAttributeEnum.HEIGHT));
					view_child = new LinkedHashMap<String, Object>();
					view.put("layer", view_child);
				}

				Object _layerId = map.get("layerId");
				if (_layerId != null) {
					String layerId = _layerId.toString();
					LinkedHashMap<String, Object> layer;
					LinkedHashMap<String, Object> layer_child;
					if (view_child.containsKey(layerId)) {
						layer = (LinkedHashMap<String, Object>)view_child.get(layerId);
						layer_child = (LinkedHashMap<String, Object>)layer.get("element");
					} else {
						layer = new LinkedHashMap<String, Object>();
						view_child.put(layerId, layer);

						layer.put("id", layerId);
						layer.put("name", map.get("layerName").toString());
						layer.put("viewSn", viewId);
					...
					Object _elementId = map.get("elementId");
					if (_elementId != null) {
						String elementId = _elementId.toString();
						...
						if(map.get("elementMap")!=null){
							Long element_mapKey = Long.valueOf(map.get("elementMap").toString());
							...
							element.put("width",element_map.get(FileAttributeEnum.WIDTH.name()));
							element.put("height",element_map.get(FileAttributeEnum.HEIGHT.name()));
							element.put("position", map.get("position").toString());
							element.put("scale", map.get("scale").toString());
						}
					}
				}
			}
			String value = JsonUtils.mapToJson(space);*/
	...
	public void QueryElement(HttpServletResponse response,
		...
		try {
			...
			for(Map<String, Object> child:list){
				...
				String elementProductSn = child.get("elementProductSn")!=null?child.get("elementProductSn").toString():"";
	...
	public void QueryElementProductInfo(HttpServletResponse response,PanoPerspectiveElementProductModel model) {
		try {
			PanoPerspectiveElementProduct product = _service.QueryElementProductInfo(model.getSn());
	...
	public void UpdateElementOrder(HttpServletResponse response,String pre,String next,String layerSn){
		try {
			JSONObject json = new JSONObject();
	...
	public void UpdateLayerOrder(HttpServletResponse response,String pre,String next){
		try {
			JSONObject json = new JSONObject();
